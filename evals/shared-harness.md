...
```sh
python3 skill-eval-harness/skill_benchmark.py validate slide-maker/evals/shared-benchmark.json
```

Prepare paired run tasks:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare slide-maker/evals/shared-benchmark.json --split tune --out /tmp/slide-maker-tasks.jsonl
```

Include ablation variants when running a focused regression check:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare slide-maker/evals/shared-benchmark.json --split tune --include-ablations --out /tmp/slide-maker-ablation-tasks.jsonl
```
...
```sh
python3 skill-eval-harness/skill_benchmark.py benchmark slide-maker/evals/shared-benchmark.json --runs slide-maker/eval-runs/latest --out /tmp/slide-maker-benchmark.json
```
