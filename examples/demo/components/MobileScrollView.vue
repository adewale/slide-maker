...
// Cache clicks contexts so they aren't recreated on every render
...
// Track scroll position for progress bar integration
const scrollContainer = ref(null)
const currentCardIndex = ref(0)

// Section boundaries (same logic as ProgressSegmentBar)
...
.mobile-segment {
  ...
  /* 44px tap target via padding, 3px visual */
  padding: 20px 0;
...
.scroll-card {
  ...
  height: 100svh;
  flex-shrink: 0;
  border-bottom: 1px solid color-mix(in srgb, var(--deck-fg, #2b2622) 8%, transparent);
  overflow: hidden;
}

/* Deck bg instead of black bars */
.scroll-card :deep(.slidev-slide-content) {
  background: var(--deck-bg, #f4f0e8) !important;
}
