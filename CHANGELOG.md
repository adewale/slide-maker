...
- **`examples/build.sh`** — retired the legacy bash builder. `tools/build.py` is now the sole builder; it now discovers personal decks in `decks/` (or `$DECKS_DIR`) automatically — the previously-stubbed external-deck path — so `new-deck.py` output builds with plain `npm run build`. The two builders had drifted twice (most recently the 404.html: build.sh still emitted the old redirect hack); consolidating to one removes the drift class. Both scaffolders (`new-deck.py` and `new-deck.sh`) no longer edit build.sh's `LOCAL_DECKS` array (that registration was already broken in both — the `durable-objects` anchor they sought didn't exist); `new-deck.sh` also dropped its macOS-only `sed -i ''`. The orphaned static `examples/index.html` (build.py generates the gallery dynamically and no longer copies it) was removed
- **`tools/new-deck.sh` and `tools/build-and-verify.sh`** — retired the legacy bash siblings; `tools/new-deck.py` and `tools/build-and-verify.py` are canonical (both verified strict supersets). `new-deck.sh` was in fact already broken (its `transitions.css` copy pointed at a nonexistent path and aborted under `set -e`) and its inlined component templates had drifted stale; `new-deck.py` copies them from the canonical `skills/slide-maker/` source. Fixed two latent bugs exposed in the survivors: `new-deck.py` now vendors `ProgressSegmentBar`/`AudienceQRCode`/`MobileScrollView` (which `global-top.vue` imports, so scaffolded decks build), and `build-and-verify.py` now discovers decks dynamically (`generated-decks/` + `decks/`) instead of a hardcoded list that named two non-existent decks (`tufte`, `durable-objects`) and missed `pdf2croissant`. Repointed docs (`LESSONS_LEARNED`, reference deck `ecosystem.md`, `COMPILER_RULES`) at the `.py` tools
- **`tools/deploy.py`** — retired the legacy GitHub-Pages-via-`gh-pages`-branch deployer. GitHub Pages now deploys through `.github/workflows/deploy-pages.yml` (Actions; the `gh-pages` branch is no longer the Pages source), and Cloudflare via `tools/deploy-cf.py`. Also corrected docs (`COMPILER_RULES.md`, eval fixture) that mislabeled `deploy.py` as the Cloudflare deployer — that is `deploy-cf.py`
