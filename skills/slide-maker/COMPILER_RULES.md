...
- [Acceptance checklist](#acceptance-checklist) → [ACCEPTANCE_CHECKLIST.md](ACCEPTANCE_CHECKLIST.md)
...
Every generated `README.md` must include these sections in order:

1. **Title + description** — deck name, what the presentation covers, link to project repo if applicable
2. **Quick start** — `npx slidev` to preview locally
3. **Build** — `npx slidev build` to produce `dist/`
4. **Share** — three options, in this order:
   - **PDF**: `npx slidev export` (or use the download button in the built deck)
   - **Deploy**: `python tools/deploy-cf.py` to publish to Cloudflare Workers (if it exists in the project); GitHub Pages deploys automatically via `.github/workflows/deploy-pages.yml` on push to `main`
   - **Manual**: `npx slidev build` then deploy `dist/` to any static host (Cloudflare Pages, Vercel, Netlify, GitHub Pages). The build output is a static SPA — the host must serve `index.html` for all sub-routes.
5. **Structure** — file tree or table showing `slides.md`, `deck.spec.md`, `styles/`, `components/`, etc.
6. **Preset** — style preset name and any project color overrides

Keep the README concise. No boilerplate, no badges, no contributing guide.
...
| `global-bottom.vue` | Deck chrome footer — slide number + deck title, hidden on cover/end/section layouts |
...
- All decks use the same `global-top.vue` shape (mobile scroll view, progress bar, keyboard help, QR code). Laser pointer is provided natively by Slidev (>=52.15.0) via the nav-bar cursor-style menu — do not re-implement it.
...
`global-bottom.vue` uses `$nav.currentLayout` to hide itself on cover, end, and section slides, `$slidev.configs.title` for the deck title, and `$nav.currentPage` / `$nav.total` for slide numbers.
...
See [SOURCES.md](SOURCES.md) for the full source-material lookup table, extraction heuristics, through-line extraction, and project identity rules.
...
- `download: true` — adds a PDF download button to the built SPA so audiences can save slides
...
- Footer elements (`global-bottom.vue`) must have `pointer-events: none` and must hide on layouts that use the full viewport height or invert the palette (cover, end, section).
...
Extracted to [ACCEPTANCE_CHECKLIST.md](ACCEPTANCE_CHECKLIST.md) — load during Phase 7 (Validate).
