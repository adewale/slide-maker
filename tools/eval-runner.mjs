...
//     graded by a sub-agent of the orchestrating agent — no API key, reuses the
//     agent's own model access:
//       --emit-judge-tasks <path>  writes per-deck grading tasks (rubric + deck
//                        file paths); the orchestrating agent dispatches one
//                        grading sub-agent per task.
//       --judge-results <path>     folds the sub-agents' verdicts into scores.
//
...
// ── Judge schema + instructions (consumed by the sub-agent handoff) ────
...
// Public rubric files — the same ones the generator compiles against. The
// sub-agent reads these directly (they're referenced by path in the tasks doc).
...
// ── Verdict merge (used by the sub-agent handoff) ──────────────────────
...
async function gradeEval(evalCase, opts) {
  ...
  let holdout = null;
  let summary = null;
  // Semantic assertions + rubric scores come from the sub-agent handoff: emit
  // tasks (--emit-judge-tasks), dispatch grading sub-agents, fold the verdicts
  // back in (--judge-results). Without verdicts, those assertions stay deferred.
  ...
  return {
    ...
    visual, slopTells, holdout, summary,
  };
}
...
function parseArgs(argv) {
  const opts = { only: new Set(), overrides: new Map(), generate: null, json: null, emitJudgeTasks: null, judgeResultsPath: null, record: false, trend: false };
...
async function main() {
  ...
  const mode = opts.judgeResults ? ', sub-agent verdicts loaded' : opts.emitJudgeTasks ? ', emitting judge tasks' : '';
  console.log(`${C.bold}${C.magenta}eval-runner${C.reset}  ${C.dim}${suite.skill_name} — ${cases.length} eval(s)${mode}${C.reset}`);

  const reports = [];
  for (const evalCase of cases) {
    reports.push(await gradeEval(evalCase, opts));
  }
  ...
  if (opts.json) {
    ...
    mkdirSync(dirname(out), { recursive: true });
    writeFileSync(out, JSON.stringify({ skill: suite.skill_name, when: new Date().toISOString(), judged: !!opts.judgeResults, reports }, null, 2));
