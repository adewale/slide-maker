...
- **12 transitions** — fade, slide-left, slide-up, iris, wipe-right, wipe-up, morph-fade, zoom-in, zoom-out, flip-x, flip-y, blur
- **v-motion** for element-level animation with per-property easing
- **v-mark** annotations — <v-mark>underline</v-mark>, circle, strike, highlight, box
- **Shiki Magic Move** for animated code transforms between states

</v-clicks>

<!-- Transitions and visual effects get their own slide because each category serves a different purpose. Transitions control slide-to-slide rhythm. v-motion controls element-level entrance and emphasis. v-mark draws the audience's eye to specific words or phrases. Shiki Magic Move animates code evolution. Together they form the motion vocabulary, but they are not interchangeable.

[click] 12 transitions — the full catalog, not "many". Specificity beats vagueness.

[click] v-motion — Slidev's @vueuse/motion integration for element animation.

[click] v-mark — the annotation system. Five styles, each with a distinct rhetorical purpose.

[click] Shiki Magic Move — animated code diffs. Show the before and after as a transformation, not a side-by-side. -->

---
transition: slide-left
---

# Data visualization and design guardrails

- **8 Mermaid diagram types** — flowcharts, sequence diagrams, mind maps, pie charts, timelines, git graphs, entity-relationship, state diagrams
- All diagrams use explicit inline styling — no default black-box labels
- **WCAG contrast checking** — 4.5:1 body text, 3:1 large text
- **LLM-tell detection** — flags stock phrases, symmetric structure, safe palettes
- **CRAP audit** — Contrast, Repetition, Alignment, Proximity
- Screenshot validation against the spec's intent

<!-- Data visualization and guardrails share a slide because they are both about the output quality pipeline. Mermaid diagrams are the primary charting tool; the inline styling rule exists because Mermaid's default rendering drops text into black rectangles — every node needs explicit fill, stroke, and color. The four guardrails form a quality gate: WCAG ensures accessibility, LLM-tell detection ensures the deck does not read as AI-generated, CRAP ensures visual design principles, and screenshot validation closes the loop between spec intent and rendered reality. No v-clicks here — the point is showing the full quality surface at a glance. -->
